...
STRATEGY_NAME = 'dispersal'

CONFIG = json.loads('{"dispersal_distance": 100.0}')

def make_policy():
    return DispersalPolicy('dispersal', **CONFIG)
...
def predict(step: StepResponse = Body(...)):
    global _policy, _last_time, _last_signature, _last_response
    with _lock:
        signature = json.dumps(step.model_dump(), sort_keys=True, separators=(",", ":"))
        if signature == _last_signature:
            return _last_response
        if _last_time is not None and step.sim_time < _last_time:
            _policy = make_policy()
        if step.game_status != "ok":
            _policy = make_policy()
            response = {"actions": []}
        else:
            states = [agent.model_dump() for agent in step.agent_status]
            response = {"actions": [a.model_dump() for a in _policy.decide_all(states, step.sim_time)]}
        _last_time = step.sim_time
