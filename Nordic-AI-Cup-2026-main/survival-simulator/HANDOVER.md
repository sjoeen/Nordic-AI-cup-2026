...
6. **Uncommitted duplicate results.** `results/index.csv` has two extra rows, and there are two extra per-seed files, all from 11:03. They are full scored runs made with the global Python and exactly repeat C1-E00 and C1-E01. They came from running the notebook interactively in an editor on the global Python kernel. That session also saved its outputs into `survival_baseline.ipynb`, which grew to about 83 MB and is uncommitted; clear the outputs before committing it. The index is append-only, so they were not deleted. Decide whether to commit them, or add an index row marking them invalid.
